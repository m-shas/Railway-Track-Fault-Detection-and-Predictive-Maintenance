...
def compute_alert_level(row) -> str:
    """Determine alert level using LSTM_RUL (primary) or RUL_Predicted_days (fallback)."""
    prob = row.get("Predicted_Failure_Prob", 0)
    # Prefer BiLSTM RUL prediction; fall back to original column
    rul  = row.get("LSTM_RUL", row.get("RUL_Predicted_days", 999))
...
def compute_alert_score(row) -> float:
    """Compute weighted urgency score 0-100.

    Uses LSTM_RUL (primary BiLSTM prediction) if available,
    otherwise falls back to RUL_Predicted_days.
    """
    prob    = row.get("Predicted_Failure_Prob", 0)
    rul     = row.get("LSTM_RUL", row.get("RUL_Predicted_days", 365))
...
def generate_alert_log(df: pd.DataFrame) -> pd.DataFrame:
    """Generate alert log with CRITICAL and WARNING rows.

    Uses CNN_Fault (primary) for fault type lookup and
    LSTM_RUL (primary) for RUL-based alert scoring.
    Falls back to Failure_Type / RUL_Predicted_days if columns absent.
    """
    alerts = df.copy()

    # Compute alert level and score (uses LSTM_RUL if present)
    alerts["Alert_Level"] = alerts.apply(compute_alert_level, axis=1)
    alerts["Alert_Score"] = alerts.apply(compute_alert_score, axis=1)

    # Expose the RUL value that was actually used for downstream display
    if "LSTM_RUL" in alerts.columns:
        alerts["RUL_Predicted_days"] = alerts["LSTM_RUL"]

    # Use CNN_Fault (primary) for maintenance rule lookup
    fault_col = "CNN_Fault" if "CNN_Fault" in alerts.columns else "Failure_Type"

    fault_desc, rec_action, base_prio = [], [], []
    for _, row in alerts.iterrows():
        ft = row.get(fault_col, "")
        if ft in MAINTENANCE_RULES:
            desc, action, prio = MAINTENANCE_RULES[ft]
        else:
            desc, action, prio = "Unknown", "Inspect and diagnose", "LOW"
    ...
    alerts["Failure_Type"]       = alerts[fault_col]  # overwrite with primary

    # Filter: keep only CRITICAL and WARNING, sort by score
