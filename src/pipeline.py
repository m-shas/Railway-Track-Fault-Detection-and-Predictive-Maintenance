...
def run_pipeline():
    ...
    print("=" * 70)
    print("RAILWAY TRACK FAULT DETECTION - FULL PIPELINE")
    print("=" * 70)
    ...
    print(f"  Dashboard: outputs/railway_dashboard.html")
...
def build_dashboard_data(df, vibr_df, alert_log,
                         ...
                         clf_model, clf_accuracy, clf_y_test, clf_y_pred,
                         encoders):
    """Build the complete dashboard_data.json payload."""
...
.header {{ background:linear-gradient(135deg,#1e3a5f,#0f1923); padding:20px 32px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; }}
.header h1 {{ font-size:1.5rem; font-weight:700; }}
.header h1 span {{ color:var(--accent-blue); }}
.header .badge {{ background:var(--accent-green); color:#fff; padding:4px 12px; border-radius:12px; font-size:0.75rem; font-weight:600; }}
.tabs {{ display:flex; background:var(--bg-secondary); border-bottom:2px solid var(--border); padding:0 24px; overflow-x:auto; }}
.tab {{ padding:12px 24px; cursor:pointer; color:var(--text-secondary); border-bottom:2px solid transparent; margin-bottom:-2px; white-space:nowrap; transition:all 0.2s; font-weight:500; }}
...
<body>
<div class="header">
  <h1>🚂 <span>Railway</span> Track Fault Detection Dashboard</h1>
  <span class="badge">AI-Powered • Real-Time</span>
</div>

<div class="tabs" id="tabs">
  <div class="tab active" data-tab="overview">📊 Overview</div>
  <div class="tab" data-tab="vibration">📳 Vibration</div>
  <div class="tab" data-tab="blocks">🛤️ Track Blocks</div>
